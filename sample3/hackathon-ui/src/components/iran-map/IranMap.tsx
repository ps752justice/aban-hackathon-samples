...
import iranProvinces, { IMapProvince } from "./data/iranProvinces";
import iranBorder, { caspianD, persianGulfD } from "./data/IranMapData";
import "./IranMap.scss";
import {
    IDeathResponse,
    IProvinceRecord,
} from "../../services/models/aban-response";
import { ModalList } from "../organisms/modal-list/modal-list";
...
const IranMap: React.FC<IranMapProps> = ({
    ...
    const [province, setProvince] = useState<IMapProvince | null>(null);
    const [provinceOnClick, setProvinceOnClick] = useState<IMapProvince | null>(
        null
    );
    const [searchParams, setSearchParams] = useSearchParams();
    const langugage = useSelector(languageSelector);

    const getDeathReportByProvince = (
        province: IMapProvince
    ): IProvinceRecord => {
        return (
            deathReport?.provinceRecords?.find(
    ...
    const getProvinceName = (province: IMapProvince | null): string => {
        if (!province) return "";
        return langugage?.code == "en" ? province.enName : province.name;
    };
    ...
    const getMapProvinceVictimsCount = (province: IMapProvince) => {
        const deathReport = getDeathReportByProvince(province);
        return deathReport?.count ?? 0;
    };

    useEffect(() => {
        const provinceIdFromUrl = searchParams.get("provinceId");
        if (provinceIdFromUrl != null) {
            const selectedProvince = provinces.find(
                (x) => x.id.toString() == provinceIdFromUrl
            );
            if (selectedProvince) {
                setProvinceOnClick(selectedProvince);
                setShowModal(true);
            }
        }
    }, [searchParams]);
    ...
    return (
        <div className="container">
            <ModalList
                ...
                provinceId={provinceOnClick?.id ?? null}
                provinceName={getProvinceName(provinceOnClick)}
                people={deathReport?.people}
                    ...
                    >
                        ...
                        <g className="province">
                            {provinces.map((province) => (
                                <g
                                    key={`container-${province.id}`}
                                    onMouseOver={() => setProvince(province)}
                                    onMouseLeave={() => setProvince(null)}
                                    onClick={() => {
                                        setProvinceOnClick(province);
                                        setShowModal(true);
                                    }}
                                >
                                    <path
                                        ...
                                        d={province.d}
                                    />
                                    {getMapProvinceVictimsCount(province) && (
                                        <g>
                                            <circle
                                                className="victims-count-circle"
                                            ...
                                            />
                                            <text
                                                className="victims-count-text"
                                                x={province.center.x}
                                                y={province.center.y}
                                                textAnchor="middle"
                                                dy=".3em"
                                            >
                                                {getMapProvinceVictimsCount(
                                                    province
                                                )}
                                            </text>
                                        </g>
                        ...
                        <g className="sea">
                            ...
                            <path className="persian_gulf" d={persianGulfD} />
                        </g>
                        <g className="lake">
                            <path
